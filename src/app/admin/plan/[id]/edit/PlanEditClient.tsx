...
export default function PlanEditClient({ plan, matrixHeight }: Props) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [name, setName] = useState(plan.name)
    const [price, setPrice] = useState(plan.price.toString())
    const [referralBonus, setReferralBonus] = useState(plan.referralBonus.toString())

    // Extract levels from JSON
    const initialLevels = Array.isArray(plan.levelCommissions)
        ? plan.levelCommissions.map(String)
        : Array(matrixHeight).fill("")

    // Ensure we have exactly matrixHeight levels
    while (initialLevels.length < matrixHeight) initialLevels.push("")
    if (initialLevels.length > matrixHeight) initialLevels.length = matrixHeight

    const [levelCommissions, setLevelCommissions] = useState<string[]>(initialLevels)
    ...
    return (
        <div className="space-y-6 max-w-6xl mx-auto">
            <div className="flex items-center justify-between">
                <h1 className="text-2xl font-semibold text-gray-800">Plan Edit</h1>
                <Link
                    href="/admin/plan"
